...
| 🌙 야간 자동 회수 | 야간 정리가 끝난 뒤 비어 있는 페이지를 정해진 몫만큼 반복해서 OS 에 돌려줌 (실행 전 디스크 여유를 스스로 확인하고, 모자라면 건너뛰고 로그만 남김) |
| 🧺 고아 기록 자동 정리 | 어느 흐름에도 붙지 못한 기록을 밤에 따로 도는 정리가 **이틀에 걸쳐 확인하고** 제거 (요약이 늦게 도착한 기록을 죽이지 않기 위한 유예). 실행 시각은 보관 기간 정리와 따로 설정 |
| 📈 적재·디스크 상태 상시 표시 | 버튼을 누르지 않아도 설정 페이지에서 적재 경합·요약 실패 누계와 **DB 크기·회수 가능한 빈 공간**을 확인 |
...
| 🚧 진입점 없는 흐름 차단 | 진입점(controller)이 아닌 곳에서 시작되려는 흐름을 만들지 않는 옵션 (**기본 꺼짐** — 켜면 진입점 밖에서 시작되는 흐름이 통째로 안 남는 대가. 배치 워커가 대표적인 예이고 메시지 소비자·기동 작업·요청에서 갈라진 별도 스레드도 포함) |
...
투명하게 적어둡니다. 운영 도입 결정 시 참고하세요. **이 절은 지금 버전의 상태입니다** — 버전별로 무엇이 언제 해소됐는지는 [CHANGELOG](./CHANGELOG.md) 를 보세요.

- **단일 서비스 추적** — 서비스 간 분산 trace 연결(MSA)은 향후 예정 (아래 로드맵)
- **동기 호출만** — `@Async` / WebFlux 비동기 경로는 아직 추적하지 않습니다
- **메서드 파라미터 이름이 `arg0`·`arg1`…** — 모든 인자를 캡처하지만, **여러분의 앱이 `-parameters` 컴파일 옵션 없이 빌드되면** 파라미터 이름 대신 인덱스로 표시됩니다. 이 옵션으로 다시 빌드하면 실제 이름이 나타납니다 — agent 가 대신 알아낼 수 있는 정보가 아니라 **앱 빌드 설정 사항**입니다
...
- **흐름의 위쪽을 빼면 아래가 조각납니다** — 계측 제외는 "그 클래스의 기록을 만들지 않는다"는 뜻이지 "그 아래 호출을 따라가지 않는다"는 뜻이 아닙니다. 조상을 빼면 그 아래 호출들이 각자 독립된 시작점이 되어 조각난 흐름이 쏟아질 수 있습니다. "진입점 없는 흐름 만들지 않기" 옵션(기본 꺼짐)으로 **완화**할 수 있지만 소멸은 아닙니다 — 옵션을 켜지 않으면 그대로이고, 켜면 **진입점 밖에서 시작되는 흐름이 통째로 안 남는** 대가가 있습니다(배치 워커가 대표적인 예이고, 메시지 소비자·기동 작업·요청에서 갈라진 별도 스레드도 포함됩니다). **빼기 전에 [계측 분석](#주요-기능--features) 화면에서 예상 결과를 먼저 확인하세요**
...
- **돌려받은 디스크 공간은 영구적이지 않습니다** — 정리·최적화로 회수한 공간은 트래픽이 늘면 다시 그만큼 채워집니다. 파일 크기를 계속 낮게 유지하려면 보관 기간을 줄이거나 계측량 자체를 줄이는 편이 근본적입니다. 오래 밀려 있던 빈 공간을 한 번에 돌려받는 절차는 [API 문서](./docs/api.md) 의 「밀린 빈 공간을 한 번에 돌려받기」 절에 있습니다
...
- **SQL 원문 표는 지금 지우지 않습니다 — 종류 수만큼 자라기만 합니다** — v0.7+ 는 span 속성 안의 SQL 원문을 별도 표에 한 번만 담습니다. 이 표는 **서로 다른 SQL 의 종류 수만큼만** 늘고 대개 배포할 때만 늘어서, 이 버전에서는 정리 기능을 두지 않았습니다(이 프로젝트 운영 환경에서 측정 시 57 종 · 43 KB — 2026-09-05 15:33). 다만 앱이 문자열을 이어 붙여 SQL 을 만들면(`IN (?,?,?)` 의 물음표 개수가 매번 다른 형태 등) 종류가 계속 늘 수 있고, 그러면 이 표는 **줄어들지 않고 자라기만 합니다**
...
- **저장량 계속 줄이기** — 옛 형태로 쌓인 행이 다 사라진 뒤 쓰지 않게 된 본문 열을 정리하고, SQL 원문 표가 계속 커지는 환경이 확인되면 그때 정리 방식을 정합니다 (위 [현재 한계](#현재-한계--current-limitations) 참조)
