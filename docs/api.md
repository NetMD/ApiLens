...
- **수동 정리(`cleanup` = 보관 기간 즉시 적용)는 고아 span 정리를 하지 않습니다.** 고아 span(요약 행이 없어 어느
  흐름에도 안 붙는 기록)은 **야간 정리에서만, 이틀에 걸쳐 확인하고 지웁니다** — 첫 밤에 후보로 적어 두고, 다음
  밤에도 여전히 고아인 것만 지웁니다. 요약이 늦게 도착한 기록을 죽이지 않기 위한 유예입니다. 다만
  `purge`(전체 삭제)는 되돌릴 수 없는 수동 조작이라 유예 없이 그 자리에서 함께 지웁니다.

---

## 밀린 빈 공간을 한 번에 돌려받기 (수동 1회 절차)

야간 정리는 한 번에 정해진 몫만 회수하므로, 오랫동안 쌓인 빈 공간은 아래 절차로 한 번 정리한 뒤 야간 정리에
