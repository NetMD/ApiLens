...
AC: AC-05-1 ~ AC-05-5
비협상: 운영망 옵션 표 5행 + PII 경고 박스 + NAS 체크리스트 — PM §0.1-(3)/(4)/(5)/(6) 인용
CLAUDE.md 룰: "아키텍처 핵심 원칙" + "절대 변경하지 말아야 할 결정 사항 §7" (포트 8765) 인용
-->

# ApiLens Agent 옵션 (v0.1)
...
| 옵션                                | 기본값                  | 필수 | 설명                                                                |
| ----------------------------------- | ----------------------- | ---- | ------------------------------------------------------------------- |
| `apilens.service.name`              | —                       | ✅   | 서비스 식별자 (한 호스트 = 한 service.name 권장)                    |
| `apilens.server`                    | `http://localhost:8765` |      | ApiLens server base URL. 끝 슬래시 무관. http/https만 허용          |
| `apilens.enabled`                   | `true`                  |      | `false` 시 agent 완전 비활성 (transport thread도 안 띄움)           |
| `apilens.sampling.rate`             | `1.0`                   |      | head-based, [0.0, 1.0]. 범위 외/parse fail → 1.0으로 fallback       |
| `apilens.batch.max-size`            | `100`                   |      | 한 HTTP POST 당 span 최대                                           |
| `apilens.batch.flush-interval-ms`   | `1000`                  |      | 강제 flush 주기 (ms)                                                |
| `apilens.queue.capacity`            | `10000`                 |      | 메모리 buffer 크기. 초과 시 silent drop (호스트 앱 보호 우선)       |
| `apilens.payload.max-bytes`         | `65536`                 |      | payload 캡처 임계. 초과 시 truncate=true로 자름                     |
| `apilens.debug`                     | `false`                 |      | `true` 시 stderr에 상세 로그                                        |
| `apilens.jdbc.capture-result-set`   | `false`                 |      | **opt-in** — true 시 JDBC SELECT 결과(row)를 payload_out에 캡처. 위험 항목 참고 |
| `apilens.jdbc.capture-params`       | `true`                  |      | **default ON** — PreparedStatement 표준 12종 setter + addBatch 호출에서 파라미터 값을 PAYLOAD IN에 직렬화. 운영망 hot-path 오버헤드 회피용 escape hatch로 `false` 토글 가능. 비활성 시 advice 자체가 weaving 되지 않아 런타임 비용 0. 자세한 동작은 아래 항목 참고 |
...
- exponential backoff / 영속 큐는 v0.2
...
## `apilens.jdbc.capture-result-set` (opt-in, v0.1.1+)
...
## `apilens.jdbc.capture-params` (default ON, Phase E3+)
...
- 모든 advice 진입점에 `try-catch(Throwable)` + silent drop — 호스트 앱
  ...
  포함). v0.1 release blocker 직전 dogfooding 12라운드 R6 회귀 진원지 봉인.
...
- 비표준 driver-specific setter (`setOracleObject`, `setPGobject`, etc.) 와
  12종 외 표준 setter (`setObject`, `setBlob`, etc.) 는 본 phase 영역 밖 —
  매처 자체가 매치하지 않아 advice 진입 0건.

### ⚠️ PII 노출 경고 (v0.1 제약, v0.2 개선 예정)
...
3. v0.2 에서 parameterIndex 키 구조에서도 작동하는 마스킹 룰 보강 예정
   (column 이름 추적 또는 value heuristic).

이 제약은 사용자 명시 비협상 결정 D-04 의 직접 결과이며 구현 버그 아닙니다.
release blocker 도 아닙니다 — 단 default ON 결정과 결합되어 인지 없이
설치하는 운영자에게 위험할 수 있으므로 본 문단 명시.

## Security 권고

> Phase G1 신설. 본 절은 운영망에 ApiLens agent 를 적용할 때의 보안 권고를 단일 위치에서 안내합니다.
...
`apilens.server` (collector endpoint) 는 NAS dogfooding 시 동일 호스트 (`http://localhost:8765`) 가정. 외부 공개 endpoint 사용 시 TLS 종단 (v0.2 `apilens.server.ca-cert` 옵션 추가 예정) 적용 후 사용 권장. v0.1 은 plain HTTP 만 지원합니다.
...
### PII 마스킹 한계 + 권장 조치 (R6 / SEC-01 cross-link)

> [PM §0.1-(6) 인용] "R6 호스트 throw 0 절대 원칙": agent 코드 모든 진입점은 `try { ... } catch (Throwable t) { silent drop }` 패턴 의무.

본 원칙으로 agent 자체 장애가 호스트 앱에 영향 0 보장. 다음 영역 모두 호스트 throw 0 단언:
...
PII 마스킹 한계는 `## ⚠️ PII 노출 경고 (v0.1 제약, v0.2 개선 예정)` 절을 참조하세요 (`apilens.jdbc.capture-params` default ON + 이름 기반 룰 미작동 위험은 v0.2 보강 예정). 운영자 권장 의사결정:
...
### Server-side 마스킹 엔진 우회 금지 (R15 / PM §0.1-(5) 인용)

> [PM §0.1-(5) 인용] "회귀 가드 grep `payloads = ` 0 hit 단언": agent 코드 어디에서도 `payloads = ` 직접 SQL 작성 금지. 모든 payload 쓰기는 server-side 마스킹 엔진을 거친다.

agent 가 payload 를 server 로 송신할 때 server 의 마스킹 엔진 (`apilens-common` 공유 엔진) 을 반드시 통과합니다. agent 자체에서 DB 의 `payloads` 테이블에 직접 INSERT 하거나 마스킹을 우회하는 경로 0건. v0.1 release 후 CI workflow 의 `regression-grep-gate` job 이 본 단언을 자동 검증합니다.

## 운영망 deployment 권장 옵션 (5 환경)

> Phase G1 신설. 본 표는 v0.1 기준이며 운영자가 자기 환경을 정확히 1개로 매핑할 수 있습니다. 매핑 불가능 시 (예: "운영망인데 PII 의심 + 고부하" 동시) → **보안 우선 row 채택** (운영망 PII 의심).

| 환경 | `apilens.jdbc.capture-params` | `apilens.jdbc.capture-result-set` | `apilens.sampling.rate` | `apilens.retention.days` | `apilens.payload.max-bytes` |
|---|---|---|---|---|---|
| 개발 (local) | `true` (default) | `true` (디버깅용 opt-in) | `1.0` | `7` | `65536` (default) |
| 스테이징 | `true` (default) | `false` (default, 검증 후 활성 가능) | `1.0` | `7` | `65536` |
| 운영망 일반 | `true` (default) | `false` | `1.0` (저트래픽 가정) | `7` (NAS 디스크 절약) | `65536` |
| 운영망 PII 의심 | **`false` (escape hatch)** | `false` | `1.0` | `7` | `65536` |
| 고부하 hot-path | **`false` (오버헤드 0)** | `false` | `0.1` 또는 그 이하 | `3` (디스크 보호) | `16384` (축소) |

### 비협상 사유 (PM §0.1-(4) 5행 인용)

- `apilens.collector.endpoint=http://localhost:8765` — NAS dogfooding 시 동일 호스트 가정
- `apilens.masking.client-side=false` — v0.1 은 server-side 마스킹 only (PM §0.1-(1)-6 인용)
- `apilens.sample.rate=1.0` — 운영자가 직접 깔아서 쓰므로 sampling 불필요 (저트래픽 가정)
- `apilens.agent.silent-on-error=true` — 호스트 throw 0 절대 원칙 (R6 회귀 진원지)
- `apilens.retention.days=7` — NAS 디스크 절약 + 운영자 즉시 분석 가정

본 표는 v0.1 기준이며 v0.2+ 에서 `apilens.masking.client-side=true` toggle 추가 시 컬럼 확장 가능.

## 운영 dogfooding 체크리스트 (NAS 등 운영망 첫 부착 시)

> Phase G1 신설. PM §0.1-(3) NAS dogfooding 6항목을 체크박스 형식으로 명문화합니다.

> [PM §0.1-(3) 인용] "NAS dogfooding 가이드 6항목" — 재서술 금지.
...
- [ ] 자세한 한계는 `## ⚠️ PII 노출 경고 (v0.1 제약, v0.2 개선 예정)` 절 참조
...
- 마스킹 룰 client-side 적용 toggle (`apilens.masking.client-side`)
- exponential backoff + persistent retry queue
- TLS 인증서 옵션 (`apilens.server.ca-cert`, …)
- agent 자체 health endpoint (UDP에 짧은 ping?)
