...
- (candidate) **공유 마스킹 엔진 ReDoS 방어의 현재 상태** — 지금은 실행 시간 상한(deadline)으로 막고 있습니다. 정규식 매칭 자체를 backtracking 없는 방식으로 바꾸는 안은 **검토만 해 본 상태**이고, 채택 여부도 시기도 정해진 것이 없습니다.
- (candidate) JDBC PreparedStatement 파라미터의 이름 기반 마스킹 보강 — 현재 PAYLOAD IN 키가 parameterIndex 라 이름 기반 룰이 매칭되지 않는 한계 개선.

## [0.6.2] - 2026-08-21

> **collector(server) 전용 라운드.** **agent 를 다시 배포할 필요가 없습니다** — agent jar 도, 대상 앱 JVM 재시작도 그대로 두세요. **스키마 마이그레이션 없음** (DB 파일 변경 0). collector jar 만 교체하고 재기동하면 됩니다. agent 버전 라벨은 계속 `0.6.0` 으로 표시되는 것이 정상입니다(이번에도 agent 를 바꾸지 않았다는 뜻).

### BREAKING CHANGES

_이 release 에는 하위호환을 깨는 변경이 없습니다._ 유지보수 상태 응답에 **필드 세 개가 뒤에 추가**됐고, 기존 필드의 이름·타입·순서·의미는 그대로입니다(삭제·타입 변경 0). 새 endpoint 도 없고 스키마 변경도 없습니다.

### Fixed

- **흐름 요약이 저장에 실패해 기록이 흐름에 안 붙던 문제** — 적재의 마지막 단계에서 흐름 요약을 저장할 때, 그 작업이 **먼저 읽고 나중에 쓰는 하나의 묶음**이라 읽는 사이에 다른 저장이 끼어들면 쓰기가 **기다리지도 않고 그 자리에서 실패**했습니다. 그러면 기록(span)은 저장됐는데 요약 행이 없어, 그 흐름이 화면 목록에 아예 안 나오고 기록만 남았습니다. 이제 요약 저장을 **묶음 밖에서** 하므로 그 실패 경로 자체가 없어집니다.
  - ⚠️ **얼마나 줄어드는지 숫자로 적지 않습니다.** 이번 라운드가 만든 것은 그것을 **재는 자**이고(아래 「Added」), 실제 값은 다음 버전이 이 계기로 잽니다.
  - 요약 저장이 다른 이유로 실패할 가능성은 남아 있습니다. 그때 서버 로그에 **근본 원인 문자열**이 함께 남도록 고쳤습니다 — 이전에는 감싸고 있는 클래스 이름만 찍혀서 원인을 가를 수 없었습니다.

### Added

- **요약 저장 실패 누계 표시** — 설정 페이지 「적재 상태」에 **요약을 저장하지 못한 흐름 수**가 함께 보입니다. 이 값이 0 이 아니면 그 사이에 흐름이 화면에서 사라진 적이 있다는 뜻이라, 옆의 `(정상)` 표시가 붙지 않습니다. 서버 메모리에만 있는 값이라 재시작하면 0 부터 다시 셉니다(0 은 정상).
- **DB 크기·회수 가능한 빈 공간 상시 표시** — 버튼을 누르지 않아도 설정 페이지에서 **DB 파일 크기**와 **회수 가능한 빈 공간**을 볼 수 있습니다. 지금까지는 [지난 데이터 정리] 같은 버튼을 눌러야만 나왔습니다.
  - 이 두 값은 조회 시점에 DB 헤더에서 읽습니다. **읽기에 실패하면 두 값만 `0 B` 로 내려가고** 일시정지·재개 같은 조작 자체는 그대로 적용됩니다(서버 로그에 경고 1줄). `0 B` 가 보이면 서버 로그를 함께 보세요.
- **야간 정리 시작 로그에 DB 크기 정보 추가** — 밤마다 정리를 시작할 때 페이지 수와 페이지 크기를 함께 남깁니다. 다음 버전이 "무엇을 줄였더니 얼마나 줄었는지" 를 되돌아볼 수 있게 하는 기록입니다.

### Changed

- **고아 기록 정리의 실행 시각을 따로 설정합니다** — 어느 흐름에도 붙지 못한 기록을 치우는 야간 정리가, 보관 기간 정리와 **별개의 시각 설정**(`apilens.retention.orphan-sweep-cron`)으로 돕니다. 기본값은 둘 다 지금과 같은 매일 04:00 이라 **동작은 그대로**입니다.
  - **왜 나눴나**: 이 정리는 "첫 밤에 후보로 적고 다음 밤에도 여전히 고아인 것만 지운다" 는 유예를 두는데, 그 「밤」이 보관 기간 정리 주기에 묶여 있었습니다. 정리 주기를 짧게 바꾸면 **유예도 함께 짧아지는데 그 사실이 어디에도 안 적혀 있었습니다.**
  - ⚠️ **유예가 시간 기반이 된 것은 아닙니다.** 유예는 여전히 "정리가 도는 횟수" 기준이라, 새로 생긴 이 시각 설정 자체를 짧게 바꾸면 유예는 똑같이 짧아집니다. 달라진 것은 ① 보관 기간 주기만 바꿔서는 안 짧아진다 ② 설정 이름이 무엇을 지배하는지 말한다 ③ 정리 사이 간격이 서버 로그에 남는다 — 이 셋입니다.

### Docs

- **`docs/api.md` 「밀린 빈 공간을 한 번에 돌려받기」 절 재구성** — 화면의 **[디스크 조각 정리(최적화)] 버튼을 먼저 시도**하도록 안내를 앞에 뒀습니다. 사유는 두 가지 구조적 차이입니다 — 버튼 경로는 디스크 여유가 모자라면 스스로 거부하거나 부분 실패를 알리고, 수집기를 멈추지 않아도 됩니다. 기존 셸 절차는 **지우지 않고 대안으로 뒤에 그대로** 뒀습니다(명령·경고 문장 무변경).
- **회수 어림값의 한계 문장 1개 추가** — 문서의 "빈 페이지 1,000개당 10~20 MB" 어림값은 안내한 명령처럼 **한 문장으로 한 번에 실행할 때는 실제보다 크게 잡는 쪽**이었습니다. 여유를 과하게 요구하는 쪽이라 안전한 방향이고, **값은 그대로** 뒀습니다.
- **README** — 기능 표에 행 3개 추가(야간 자동 회수 · 고아 기록 자동 정리 · 적재·디스크 상태 상시 표시), 「현재 한계」에 항목 2개 추가(회수한 공간은 영구적이지 않음 · 고아 기록이 생기는 것 자체는 못 막음).
- **`CHANGELOG` 표기 정리** — 과거 릴리스 블록에 적혀 있던 특정 운영 서비스 이름을 일반적인 표현으로 바꿨습니다(다른 문면·버전 문자열 무변경). 미출시 절의 마스킹 엔진 후보 항목은 **약속으로 읽히던 말을 빼고 지금 상태만** 적도록 고쳤습니다.
- **버전 0.6.1 → 0.6.2** (collector jar / UI 표시 라벨). **agent 버전은 `0.6.0` 그대로입니다** — 이번에도 agent 를 바꾸지 않았기 때문이며, 서비스 목록의 agent 버전 라벨이 `0.6.0` 으로 보이는 것이 정상입니다.
...
- **운영 영향 (하위호환이지만 조치 필요)** — v0.1 이후 처음으로 **agent jar 산출물 자체가 바뀝니다**(계측 옵션 추가 + 공유 마스킹 엔진 `apilens-common` 승격에 따른 agent 재빌드 + agent 버전 0.4.0 정렬). 이번 변경(새 계측 옵션·버전 라벨)을 반영하려면 운영망(운영 서비스 1곳 등)에 **새 agent jar 를 다시 배포하고 대상 앱 JVM 을 재시작**해야 합니다. 재배포하지 않아도 구 agent 는 기존대로 동작하지만(무파손), 새 계측 제외 옵션은 새 jar 에서만 유효합니다.
