...
> **collector(server) 전용 라운드.** **agent 를 다시 배포할 필요가 없습니다** — agent jar 도, 대상 앱 JVM 재시작도 그대로 두세요. agent 버전 라벨은 계속 `0.6.0` 으로 표시되는 것이 정상입니다.
> ⚠️ **이 버전은 기동할 때 DB 스키마를 바꿉니다** (스키마 버전 5 → 7). **되돌리는 마이그레이션이 없습니다** — 올리기 전에 수집기를 정상 종료하고 `apilens.db` 를 **반드시 백업**하세요. 직전 버전의 「스키마 마이그레이션 없음」과 **반대**입니다.

### BREAKING CHANGES

- **DB 스키마가 바뀌고, 되돌릴 수 없습니다.** 기동할 때 마이그레이션 두 개가 자동으로 적용됩니다 — 본문을 담는 표 하나, SQL 원문을 담는 표 하나, 그리고 `payloads` 에 열 하나(`body_hash`)가 추가됩니다. **되돌리는 마이그레이션은 제공하지 않습니다.** 이전 상태로 돌아가는 길은 **올리기 전에 떠 둔 백업을 복원하는 것뿐**입니다.
  - 이미 쌓인 행은 **한 줄도 바꾸지 않습니다**(옛 행을 새 형태로 옮기는 작업 없음). 옛 형태 행은 보관 기간이 지나면 평소처럼 사라집니다.
- **이전 버전 jar 로 되돌리면 이 버전이 저장한 내용이 화면에서 빈 값으로 보입니다.** 이 버전이 저장한 본문과 SQL 원문은 이전 jar 가 읽는 자리에 없기 때문입니다 — **오류가 나는 것이 아니라 조용히 빈 값**이 됩니다. 새 표에 쌓인 본문도 이전 jar 는 지우지 않습니다. **되돌릴 때는 jar 만 바꾸지 말고 백업을 함께 복원하세요.**
- **API 계약은 안 깨집니다** — 경로·요청·응답에서 삭제되거나 타입이 바뀐 필드가 **0**이고 새 endpoint 도 **0**입니다. 계측 분석 응답의 요약에 필드 하나가 **맨 뒤에 추가**됐을 뿐입니다. 추적 상세의 SQL(`db.statement`)과 본문 필드는 **문자열 그대로** 나갑니다 — 저장 방식만 바뀌었고 밖으로 나가는 모양은 그대로입니다. 화면 코드도 이 축에서는 바뀐 것이 없습니다.

### Added

- **같은 본문을 저장소 전체에서 한 번만 저장합니다** — 요청·응답 본문(payload)을 내용으로 만든 지문(SHA-256)으로 식별해 **본문은 별도 표에 한 번만** 두고, 기존 행은 그 지문만 갖습니다. 같은 내용이 다시 들어오면 본문은 다시 저장되지 않습니다.
  - 지문은 **마스킹을 거치고 상한으로 자른 뒤의 저장 바이트**로 만듭니다. 마스킹 전 원문을 지문 재료로 쓰거나 저장하는 경로는 없습니다.
  - 화면에 보이는 값은 이전과 같습니다 — 읽을 때 본문을 되돌려 붙입니다.
- **span 속성 안의 SQL 원문도 한 번만 저장합니다** — 같은 SQL 이 span 마다 통째로 들어가던 것을 별도 표에 한 번만 두고, 속성에는 내부 참조만 남깁니다. **밖으로 나가는 응답은 이전과 같은 모양**입니다(읽을 때 원문으로 되돌립니다).
- **아무 행도 안 가리키는 본문을 자동으로 치웁니다** — 밤 정리가 끝난 뒤와 「전체 지우기」 뒤에 각각 한 번 돕니다. 몇 건을 지웠는지·얼마나 걸렸는지가 수집기 로그에 한 줄 남습니다(0 건이어도 남습니다).
  - 이 정리가 실패해도 **뒷정리(시각 기록·공간 회수·WAL 줄이기)는 그대로 돕니다.** 실패는 로그 한 줄로 남고 다음 밤이 이어서 지웁니다 — **긴급 대응 대상이 아닙니다.**
  - 한 번에 지우는 양에 상한이 있어 남으면 다음 실행이 이어 갑니다. 그때도 로그 한 줄이 남습니다.
- **계측 분석 요약에 「고유 본문 합」 한 줄** — 같은 조회 구간 안에서 **같은 본문을 한 번만 센 바이트 합**입니다. 순위 표의 본문 용량은 **지금까지와 똑같이 참조당**으로 둡니다(두 값의 잣대가 다르므로 섞어 읽지 마세요).
- **수집기 로그에 외부에서 온 값이 그대로 찍히지 않습니다** — 인증 없이 열려 있는 적재 입구와 그 예외 경로의 로그 인자를 감쌌습니다. 줄바꿈이 든 값이 들어와도 **로그가 한 줄로 유지**됩니다. 로그 줄의 앞머리와 필드 이름은 **안 바꿨습니다**(과거 로그와의 대조가 끊기지 않게).

### Changed

- **올린 뒤 약 이틀 동안은 옛 형태 행과 새 형태 행이 섞여 있습니다** — 두 형태를 모두 읽는 갈래가 들어 있어 **화면에서는 구분 없이 똑같이** 보입니다. 옛 형태 행은 보관 기간이 지나면 사라지고, 그때부터는 새 형태만 남습니다.
- **버전 0.6.3 → 0.7.0** (collector jar). 화면에 뜨는 버전 라벨은 `v0.6` → **`v0.7`** 입니다(잔글자까지는 라벨에 안 싣습니다). **agent 버전은 `0.6.0` 그대로**입니다 — 이번에도 agent 를 바꾸지 않았기 때문이며, 서비스 목록의 agent 버전 라벨이 `0.6.0` 으로 보이는 것이 정상입니다.
- **적재 트랜잭션은 쓰기로 시작합니다** — SQL 원문을 저장하는 단계가 「이미 있는지 먼저 읽고 없는 것만 넣는」 순서였다면, 밤 정리와 같은 순간에 겹칠 때 SQLite 가 기다리지 않고 바로 실패시켜 그 청크가 버려집니다(WAL 의 스냅샷 규칙). 그래서 읽지 않고 바로 넣습니다(`INSERT OR IGNORE`). 이 버전을 운영에 올린 첫 밤에 관측해 **출고 전에** 고쳤습니다. 겹치면 이전처럼 최대 10초를 기다립니다.
- **본문 자동 정리 로그가 회전마다 한 줄 더 남습니다** — `payload body gc round: round= deleted= acquireMs= statementMs= commitMs=` (커넥션 확보 · 문장 · 커밋에 걸린 ms). 시작 시각 줄(`payload body gc start:`)도 남습니다. 요약 줄(`payload body gc: deleted= rounds= elapsedMs=`)의 문면은 그대로입니다.

### Notes

- **얼마나 줄어드는지는 「응답 내용이 얼마나 자주 바뀌는가」에 달려 있습니다.** 같은 내용이 반복해서 들어오는 환경일수록 많이 줄고, 매번 다른 내용이 들어오면 거의 안 줄어듭니다. 이 릴리스는 **줄어드는 비율을 약속하지 않습니다** — 각자의 환경에서 재 보셔야 합니다.
  - 참고로 **이 프로젝트의 운영 환경에서 측정 시**(2026-09-05 15:33 · 보관 기간 1일 · 폴링 응답이 대부분인 환경), 저장된 본문 합 1,265,753,375 B / 559,187 행 중 **서로 다른 내용만 세면 3,678,347 B** 였고, span 속성 안의 SQL 은 **인용 바이트 171,716,209 B 에 서로 다른 원문 57 종(43,248 B)** 이었습니다. **이 값은 그 시점에 쌓여 있던 데이터를 잰 것이지 이 릴리스가 줄여 주는 양이 아닙니다.**
- **올린 뒤 약 이틀 동안은 「고유 본문 합」이 실제보다 크게 보입니다** — 옛 형태 행은 행마다 별개 본문으로 세기 때문입니다. 틀리는 방향은 **절감이 덜 되어 보이는 쪽**이고, 옛 형태 행이 사라지면 저절로 정확해집니다.
- **SQL 원문 표는 이번 버전에서 지우지 않습니다** — 서로 다른 SQL 의 **종류 수만큼만** 늘고 대개 배포할 때만 늡니다. 다만 앱이 문자열을 이어 붙여 SQL 을 만들면(`IN (?,?,?)` 의 물음표 개수가 매번 다른 형태 등) 종류가 계속 늘 수 있습니다. 그 경우 이 표는 **자라기만 합니다.**
- **파일 크기가 그날 바로 줄지는 않습니다** — 줄어드는 것은 **새로 들어가는 양**입니다. 이미 쓴 페이지는 밤 정리와 빈 공간 회수가 평소 속도로 돌려줍니다.

### Docs

- **README** — 「현재 한계」에 두 줄을 **추가만** 했습니다(절감이 응답 내용 변화 빈도에 달려 있다는 점 · SQL 원문 표가 자라기만 한다는 점). 「주요 기능」에 한 줄, 「향후 예정」에 한 줄을 더했습니다. 기존 항목은 지우지 않았습니다.
...
_이 release 에는 하위호환을 깨는 변경이 없습니다._ API 경로·요청·응답의 **필드 구성이 그대로**입니다(추가·삭제·타입 변경 0). 새 endpoint 도 없고, 스키마 변경도 없으며, 화면은 한 줄도 바뀌지 않았습니다. 새 설정 항목도 없어 설정 파일을 손댈 필요가 없습니다. 순증한 것은 **수집기 로그 두 줄**(정상 경로 1 · 실패 경로 1)뿐이고, 둘 다 기존 로그 줄의 문면을 건드리지 않습니다.

### Added

- **수집이 끊겼다 이어진 구간을 수집기 로그 한 줄로 남깁니다** — 직전에 마지막으로 받은 시각과 그 사이 공백(밀리초), 서비스 이름이 함께 적힙니다(`ingest resumed:` 로 시작하는 줄). 공백이 10분 미만이거나 처음 등록된 서비스면 줄이 안 남으며, 둘 다 정상입니다. 이 조회가 실패해도 수신은 그대로 진행되고 실패 사실만 한 줄 남습니다.
  - ⚠️ **나중에 되짚기 위한 기록이지, 무슨 일이 나면 알려 주는 장치가 아닙니다.** 수집기가 아예 꺼져 있던 구간은 **다시 켜진 뒤에야** 드러납니다 — 그 표시를 만들어 주는 것이 바로 그 프로그램이기 때문입니다. **이 release 는 그 부분을 못 닫습니다.**
  - 줄 앞머리 `ingest resumed:` 는 고정입니다. 새 항목이 생기면 뒤에만 붙습니다(과거 로그와의 대조가 끊기지 않게).

### Changed

- **평소 검사망(CI)이 출고 경로와 같은 명령으로 돕니다** — 화면 빌드 결과를 통합 단계로 넘기고, 그 단계의 빌드 명령을 출고와 같은 것으로 바꿨습니다. 화면 자원이 jar 에 안 들어가면 **경고가 아니라 실패**로 잡습니다. 화면 문법 검사·화면 시험·화면 빌드도 이제 실패가 실패로 뜹니다(전에는 실패해도 통과 처리됐습니다). **제품 동작 변화 0** — 같은 코드가 평소 검사는 통과하고 출고에서만 깨지던 구조를 없앤 것입니다.
- **회수 가능한 빈 공간 값을 자동 시험이 실제로 대조합니다** — 이 값을 확인하던 시험이 빈 공간 0 인 상태만 보고 있어 값이 잘못 계산돼도 통과했습니다. 이제 빈 페이지를 실제로 만든 뒤 대조하고, 옆의 DB 크기 값도 함께 봅니다(한쪽만 보면 관측 실패로 생긴 0 과 진짜 0 을 못 가릅니다). **제품 코드는 무변경**이고 앞으로의 회귀를 잡는 그물만 채웠습니다.
- **바깥에서 내부 저장용 설정 키를 쓰려는 요청이 400 으로 거부되는지 시험이 지목해 확인합니다** — 동작은 이전과 같습니다(허용 키 목록 무변경). 허용 목록을 넓히는 미래 변경이 조용히 지나가지 않게 만든 것입니다.
- **버전 0.6.2 → 0.6.3** (collector jar). **화면에 뜨는 버전 라벨은 `v0.6` 그대로입니다** — 잔글자까지는 라벨에 안 싣기 때문입니다. **agent 버전은 `0.6.0` 그대로입니다** — 이번에도 agent 를 바꾸지 않았기 때문이며, 서비스 목록의 agent 버전 라벨이 `0.6.0` 으로 보이는 것이 정상입니다.

### Notes

- **보관 기간 야간 정리(오래된 기록을 지우는 배치 루프)가 한 번에 다 못 지우고 끝나는 경로는 정상 경로에 없습니다** — 코드를 읽어 확인했고(그 루프의 종료 조건이 대상 소진 하나입니다), 관측된 정리는 전부 마지막 묶음이 묶음 크기보다 작았습니다. ⚠️ **같은 시각에 도는 고아 기록 스윕은 여기에 해당하지 않습니다** — 그쪽은 한 번에 살펴보는 후보 수에 상한(500)이 있어 **한 밤에 다 안 끝나는 것이 정상**입니다. **코드 변경 0**입니다. ⚠️ 이 판정에는 전제가 있습니다 — 정리가 도는 동안 **수집기 프로세스가 살아 있어야 하고** 정리 중에 예외가 나지 않아야 합니다. 그 전제가 깨지는 빈도는 재지 않았습니다. 전제를 떼고 "정리는 안전하다" 로 옮겨 읽지 마세요.
- **어느 날 정리가 지운 양이 앞뒤 날과 크게 벌어진 사례를 한 번 관측했고, 원인은 아직 못 밝혔습니다.** 설명 두 가지를 세웠다가 둘 다 기각했습니다 — 하나는 코드로(끊겼다 이어질 때 밀린 전송이 한꺼번에 오는 구조가 아닙니다), 하나는 산술로(들어오는 양의 하루 굴곡으로는 일부만 설명되고 나머지가 남습니다). **"설명됐다" 나 "이상이 아니다" 로 등급을 올려 적지 않습니다.** 위 Added 의 새 로그 줄이 몇 주 쌓여야 답을 볼 수 있습니다.

### Docs

- **README** — 「현재 한계」에 항목 1개를 **추가만** 했습니다(수집기가 꺼져 있는 동안에는 그 사실을 스스로 알릴 수 없다는 점). 기존 항목·로드맵·기능 표는 손대지 않았습니다.
